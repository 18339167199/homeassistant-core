"""Test the lmb_integration config flow."""
...
async def test_config_flow(
    ...
    result = await hass.config_entries.flow.async_configure(
        result["flow_id"],
        {"name": "My lmb_integration", "entity_id": input_sensor_entity_id},
    )
    await hass.async_block_till_done()

    assert result["type"] is FlowResultType.CREATE_ENTRY
    assert result["title"] == "My lmb_integration"
    assert result["data"] == {}
    ...
    assert config_entry.title == "My lmb_integration"
...
async def test_options(hass: HomeAssistant, platform) -> None:
    ...
    state = hass.states.get(f"{platform}.my_lmb_integration")
